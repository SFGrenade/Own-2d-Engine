...
Logic::Logic()
    : logger_( spdlog::get( "TSrv" ) ),
      network_Logic_Send_( "inproc://logic", true, &global_inproc_context_ ),
  ...
  this->logger_->trace( fmt::runtime( "[thread {:s}] Logic()" ), getThreadId() );
...
void Logic::add_Get_Config_callback( std::function< void( SFG::Proto::InProc::Get_Config_Reply const& ) > callback ) {

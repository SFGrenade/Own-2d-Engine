...
std::vector<KeyCallback> InputHandler::keyUpCallbacks =
    std::vector<KeyCallback>();
std::vector<MouseButtonCallback> InputHandler::mouseButtonCallbacks =
    std::vector<MouseButtonCallback>();
std::vector<MouseMotionCallback> InputHandler::mouseMotionCallbacks =
    std::vector<MouseMotionCallback>();
std::vector<MouseWheelCallback> InputHandler::mouseWheelCallbacks =
    std::vector<MouseWheelCallback>();
std::vector<QuitEventCallback> InputHandler::quitEventCallbacks =
    std::vector<QuitEventCallback>();
std::vector<WindowEventCallback> InputHandler::windowEventCallbacks =
...
void InputHandler::CheckInputs() {
    ...
    while (SDL_PollEvent(&InputHandler::sdlEvent) != 0) {
        if (InputHandler::quitEventCallbacks.size() &&
        ...
        } else if (InputHandler::windowEventCallbacks.size() &&
                   InputHandler::sdlEvent.type == SDL_WINDOWEVENT) {
            for (auto callback : InputHandler::windowEventCallbacks)
                callback(InputHandler::sdlEvent.window);
        } else if (InputHandler::keyDownCallbacks.size() &&
                   InputHandler::sdlEvent.type == SDL_KEYDOWN) {
            for (auto callback : InputHandler::keyDownCallbacks)
                callback(InputHandler::sdlEvent.key);
        } else if (InputHandler::keyUpCallbacks.size() &&
                   InputHandler::sdlEvent.type == SDL_KEYUP) {
            for (auto callback : InputHandler::keyUpCallbacks)
                callback(InputHandler::sdlEvent.key);
        } else if (InputHandler::mouseWheelCallbacks.size() &&
                   InputHandler::sdlEvent.type == SDL_MOUSEWHEEL) {
            for (auto callback : InputHandler::mouseWheelCallbacks)
                callback(InputHandler::sdlEvent.wheel);
                   ...
                   (InputHandler::sdlEvent.type == SDL_MOUSEBUTTONDOWN ||
                    InputHandler::sdlEvent.type == SDL_MOUSEBUTTONUP)) {
            for (auto callback : InputHandler::mouseButtonCallbacks)
                callback(InputHandler::sdlEvent.button);
                   ...
                   (InputHandler::sdlEvent.type == SDL_MOUSEBUTTONDOWN ||
                    InputHandler::sdlEvent.type == SDL_MOUSEMOTION)) {

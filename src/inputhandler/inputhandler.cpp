...
namespace SFG {
    ...
    void InputHandler::CheckInputs() {
        ...
        while (SDL_PollEvent(&InputHandler::sdlEvent) != 0) {
            ...
            else if ( InputHandler::mouseButtonCallbacks.size() && (InputHandler::sdlEvent.type == SDL_MOUSEBUTTONDOWN || InputHandler::sdlEvent.type == SDL_MOUSEBUTTONUP) ) {
                for ( auto callback : InputHandler::mouseButtonCallbacks ) callback(InputHandler::sdlEvent.button);
            }
            else if ( InputHandler::mouseMotionCallbacks.size() && (InputHandler::sdlEvent.type == SDL_MOUSEBUTTONDOWN || InputHandler::sdlEvent.type == SDL_MOUSEMOTION) ) {

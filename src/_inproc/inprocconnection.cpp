...
InProcConnection::InProcConnection( std::string const& host, bool server )
    : logger_( spdlog::get( "TSrv" ) ), network_( host, server, &InProcConnection::networkContext_ ) {
...
//   logger_->trace( fmt::runtime( "[thread {:s}] sendInProcTestThis(std::string const& t1 = \"{:s}\", std::string const& t2 = \"{:s}\")" ),
//                   getThreadId(),
//                   t1,
//                   t2 );
//
...
//   logger_->trace( fmt::runtime( "[thread {:s}] onInterThreadFunctionCall(SFG::Proto::InProc::TestThis const& msg = \"{:s}\"/\"{:s}\")" ),
//                   getThreadId(),
//                   msg.username(),
//                   msg.msgtext() );
//

...
Button::Button( std::function< void() > const& callback, std::string const& text, Widget* parent, SDL_FRect position )
    : _base_( text, parent, position ), logger_( SFG::Own2dEngine::Logger::LoggerFactory::get_logger( "Button" ) ), callback_( callback ) {
  logger_->trace( "Button( callback )" );

  logger_->trace( "Button()~" );
}
...
void Button::OnRender( SDL_Renderer* renderer ) {
  ...
  if( drawBackground ) {
    if( mbDown_ ) {
      SDL_SetRenderDrawColor( renderer, backgroundColourPressed_.r, backgroundColourPressed_.g, backgroundColourPressed_.b, backgroundColourPressed_.a );
    } else if( cursorHovering_ ) {
      SDL_SetRenderDrawColor( renderer, backgroundColourHovering_.r, backgroundColourHovering_.g, backgroundColourHovering_.b, backgroundColourHovering_.a );
    }
    SDL_RenderFillRectF( renderer, &global_position_ );
  }

  bool drawBorder = true;
  if( drawBorder ) {
    if( IsActiveHierarchy() ) {
      SDL_SetRenderDrawColor( renderer, borderColourActive_.r, borderColourActive_.g, borderColourActive_.b, borderColourActive_.a );
    } else {
      SDL_SetRenderDrawColor( renderer, borderColourInactive_.r, borderColourInactive_.g, borderColourInactive_.b, borderColourInactive_.a );
    }
